...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
  ...
  private static class MapClass extends
    ...
    public void setup(Context context) throws IOException {
      ...
      sources = conf.get("source.nodes").split(",");
    }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      if (node.getAdjacenyList().size() > 0) {
        ...
        for (int i = 0; i < num; i++) {
          node.setCurPoint(i);
          flag |= (node.getPageRank() != Float.NEGATIVE_INFINITY);
        }
        if (flag) {
          ...
          for (int i = 0; i < num; i++) {
            node.setCurPoint(i);
            mass[i] = node.getPageRank() - (float) StrictMath.log(list.size());  
          }
          ...
          for (int i = 0; i < list.size(); i++) {
            ...
            for (int j = 0; j < num; j++) {
              intermediateMass.setCurPoint(j);
              intermediateMass.setPageRank(mass[j]); 
            }
  ...
  private static class ReduceClass extends
      ...
      while (values.hasNext()) {
        ...
        if (n.getType().equals(PageRankNode.Type.Structure)) {
          ...
          for (int i = 0; i < num; i++) {
            ...
            mass[i] = sumLogProbs(mass[i], n.getPageRank());
    ...
    public void cleanup(Context context) throws IOException {
      ...
      // LOG.info("--------------------totalMass:  " + StrictMath.exp(totalMass));
      out.close();
    }
  }
  ...
  private void phase2(int i, int j, float[] missings, String basePath, int numNodes, String sources) throws Exception {
    Job job = Job.getInstance(getConf());
